...
export default class UserAccess extends Component {
	...
	onClickUbah = e => {
		...
		API.get(`${API_SERVER}v1/access/id/${userId}`).then(res => {
			if (res.status === 200) {
				this.setState({
					isModalUbah: true, userId: userId, userNama: userNama,
		...
		const ListAccess = ({ items }) => {
			if (items.length === 0) {
				return (
					<tbody>
						<tr>
							<td colSpan="13">There is no user access data</td>
						</tr>
					</tbody>
				);
			} else {
							...
							<div className="main-body">
                                ...
                                Add New
											...
											<Modal show={this.state.isModalUbah} onHide={this.handleModalUbah}>
												<Modal.Body>
													<Modal.Title className="text-c-purple3 f-w-bold">Change User Access</Modal.Title>
													<div style={{ marginTop: '20px' }} className="form-group">
											...
											<Modal show={this.state.isModalHapus} onHide={this.handleModalHapus}>
												<Modal.Body>
													<Modal.Title className="text-c-purple3 f-w-bold">Remove User Access</Modal.Title>
													<div style={{ marginTop: '20px' }} className="form-group">
														<p className="f-w-bold">Are you sure you want to remove this user's access ?</p>
													</div>
													<button style={{ marginTop: '50px' }} type="button"
