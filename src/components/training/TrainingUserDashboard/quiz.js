...
class TrainingUserDashboardQuiz extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedIndex:null,
  ...
  getDataUserDashboard (tab) {
    ...
    if(tab === 'Quiz'){
      urlHitPoint = `${API_SERVER}v2/training/exam-by-user/${idTrainingUser}/${idCompany}/${0}`;
    }else{
      urlHitPoint = `${API_SERVER}v2/training/exam-history-by-user/${idTrainingUser}/${idCompany}/${0}`
    }
  ...
  componentDidUpdate(){
    this.props.socket.on('WEB::read_training_user_dashboard', (data) => {
      ...
      if (data.user_id === idTrainingUser && data.company_id == idCompany) {
        if (data.action === 'Quiz') {
          this.getDataUserDashboard('Quiz');
        }
      }
    });
  }
                              ...
                              <div className="card-columns">
                                {listHistoryUser.length ? 
                                  listHistoryUser.map((history, index) => {
                                    ...
                                    if(this.state.selectedTab === 'Quiz') {
                                      return (
...
const TrainingUserDashboardQuiz_view = (props) => (
  <SocketContext.Consumer>{(socket) => <TrainingUserDashboardQuiz {...props} socket={socket} />}</SocketContext.Consumer>
);
export default TrainingUserDashboardQuiz_view;
